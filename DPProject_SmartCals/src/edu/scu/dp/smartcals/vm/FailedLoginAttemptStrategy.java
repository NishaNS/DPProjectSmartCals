...
public class FailedLoginAttemptStrategy implements LoginCheckPointStrategy {
	...
	public boolean performSecurityCheck(String username) {
		try {
			AdminLoginModel adminLoginModel = adminLoginDao.getLoginDetails(username);
			if(adminLoginModel.getLoginAttempts() > FAILED_LOGIN_COUNT)
				System.out.println("You have failed your login attempt for 3 times consecutively. Your account will be locked for 30 minutes.");
